Headers = {
	...
	B1Length : new Buffer([0x12, 0x00]) #TODO calculated from remaining bytes in packet
	CipSequence : new Buffer([0x62, 0x00])
	CipService : new Buffer([0x52])
	RequestPathSize : new Buffer([0x04]) #in WORDS (Double byte) TODO number of words for reques path padding appended to end of needed
	PathSegment : new Buffer([0x91]) #ANSI Extended Symbol Segment
	DataSize : new Buffer([0x06]) #TODO length of TagSymbol
	TagSymbol : new Buffer([0x43, 0x6f, 0x6e, 0x66, 0x69, 0x67])
	RequestTotalData : new Buffer([0x01, 0x00])
	...
	CipBuffer : () ->
          Buffer.concat([@CommandUnitData, @Length, @SessionHandle, @CipStatus, @SenderContext, @Options, @InterfaceHandle, @CipTimeOut, @CIPItemCount, @ConnectionAddressItem, @ConnectionIDLength, @ConnectionID, @TypeIDb1, @B1Length, @CipSequence, @CipService, @RequestPathSize, @PathSegment, @DataSize, @TagSymbol, @RequestTotalData, @RequestDataLength])
